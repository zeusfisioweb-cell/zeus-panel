...
describe('portal booking appointments POST', () => {
    ...
    it('returns 409 when slot is not available in schedule engine', async () => {
        checkRateLimitMock.mockResolvedValue({
            success: true,
            limit: 12,
            remaining: 11,
        ...
        const adminClient = {
            rpc: vi.fn().mockResolvedValue({
                data: [{ slot_start: '2030-01-01T08:00:00.000Z', slot_end: '2030-01-01T09:00:00.000Z' }],
                error: null,
            }),
            from: vi.fn((table: string) => {
                if (table === 'patients') {
        ...
        expect(body).toEqual({ error: 'slot_taken' });
